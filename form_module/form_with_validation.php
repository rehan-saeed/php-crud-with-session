...
$nameErr = $emailErr = $genderErr = $websiteErr = $passwordErr= $phone="";
$name = $email = $gender = $comment = $website = $password= $phoneErr="";
...
if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
  ...
  if (empty($_POST["phone"])) {
    ...
    // check if name only contains letters and whitespace
    if (!preg_match("/^(\+)[0-9]{5}-[0-9]{7}$/",$phone)) {
      $phoneErr = "follow this format +92000-0000000";
    }
  }
